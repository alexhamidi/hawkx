...
def fetch_all_tweets(screen_name: str, limit: int = 3200) -> list[dict]:
    ...
    page = 0
    empty_streak = 0
    out_path = os.path.join(os.path.dirname(__file__), "willccbb_posts.json")
...
print("Fetching @willccbb tweets…")
posts = fetch_all_tweets("willccbb", limit=3200)
with open(out_path, "w") as f:
    json.dump(posts, f, indent=2, ensure_ascii=False)
